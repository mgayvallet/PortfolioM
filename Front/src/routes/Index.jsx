  ...
  return (
    <>
      ...
      <main className="relative min-h-screen text-white font-sans" role="main">
        <header className="h-screen flex flex-col justify-center items-center text-center px-4" role="banner">
          ...
          <p ref={typingRef} aria-label="Typing description" className="text-xl text-gray-400 mt-3" />
        </header>

        <section className="px-6 sm:px-10 py-20 max-w-4xl mx-auto reveal" aria-labelledby="about-heading">
          ...
          <p className="text-lg leading-relaxed text-center text-gray-300">
            Passionate web developer specializing in modern and responsive interfaces. I enjoy transforming ideas into concrete products by combining design and performance. Currently seeking new challenges to continue learning and innovating.
          </p>
        </section>

        <section className="py-16 px-6 sm:px-10 max-w-5xl mx-auto reveal" aria-labelledby="skills-heading">
          <h2 id="skills-heading" className="text-3xl font-bold mb-10 text-center">Technical Skills</h2>
          <ul className="grid grid-cols-2 sm:grid-cols-3 gap-6 text-sm text-gray-300" role="list">
            {['HTML', 'React.js', 'Tailwind CSS', 'JavaScript', 'Node.js', 'Git / GitHub', 'API REST'].map((skill) => (
              <li key={skill} className="hover:scale-105 transition-transform duration-300" role="listitem">
                🔹 {skill}
              </li>
            ))}
          </ul>
        ...
        <section className="py-16 px-6 sm:px-10 max-w-7xl mx-auto reveal" aria-labelledby="projects-heading">
          <h2 id="projects-heading" className="text-4xl font-bold mb-16 text-center">Featured Works</h2>
            ...
            {works.map((title, index) => (
              ...
              >
                <div className="flex flex-col h-full w-full rounded-3xl">
                  <div className="relative w-full" style={{ height: '240px' }} role="img" aria-label={`${title} preview image`}>
                    <div
                  ...
                  <div className="px-5 pt-4 text-left">
                    <h3 id={`project-${index}`} className="text-xl font-semibold">{title}</h3>
                    <p className="text-sm text-gray-400 mt-1">{descriptions[title]}</p>
                  </div>
                  <div className="flex flex-wrap gap-2 px-5 mt-4">
                    {title === 'Portfolio' ? (
                      <>
                        <span className="bg-cyan-400 text-black text-xs font-medium px-3 py-1 rounded-full">React</span>
                        <span className="bg-indigo-500 text-white text-xs font-medium px-3 py-1 rounded-full">Tailwind</span>
                      </>
                    ) : (
                      <>
                        <span className="bg-orange-400 text-black text-xs font-medium px-3 py-1 rounded-full">HTML</span>
                        <span className="bg-blue-500 text-white text-xs font-medium px-3 py-1 rounded-full">CSS</span>
                        <span className="bg-yellow-300 text-black text-xs font-medium px-3 py-1 rounded-full">JavaScript</span>
                      </>
                    )}
                  </div>
                  <a
        ...
        <section className="py-16 reveal text-center" aria-labelledby="contact-heading">
          <h2 id="contact-heading" className="text-3xl font-bold mb-6">Contact</h2>

          {messageSent ? (
            <p className="text-green-400 text-lg font-semibold animate-pulse" role="status" aria-live="polite">
            ...
            >
              ...
              <button
                ...
                className={`w-full py-3 rounded font-semibold transition text-white
                  ${btnGreen ? 'bg-green-600' : sending ? 'bg-gray-600 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}
                `}
              >
